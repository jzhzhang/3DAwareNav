...
class Node:
    def __init__(self, mark_value,min_value,max_value, color, parent, left=None, right=None,isEdge=False):
    ...
    def __eq__(self, other):
        ...
        if self.parent is None or other.parent is None:
            ...
            parents_are_same = self.parent.mark_value == other.parent.mark_value and self.parent.color == other.parent.color
        return self.mark_value == other.mark_value and self.color == other.color and parents_are_same
...
class RedBlackTree:
    ...
    NIL_LEAF = Node(mark_value=None,min_value=None,max_value=None, color=NIL, parent=None)

    def __init__(self,block_threshold):
        self.count = 0
        self.root = None
        self.block_num=10
        self.max_distance=block_threshold
        self.ROTATIONS = {
    ...
    def bulid_connection_left_to_right(self,left_node,right_node):
        left_node.big_node=right_node
        right_node.small_node=left_node
        left_node.set_list[2]=right_node.set_list[0]
        right_node.set_list[1]=left_node.set_list[0]
        


    def add(self,value):

        node,find_node=self.insert_node(value)
        if find_node and not node.isEdge:
            return node
        node.isEdge=False
        count_iter=0

        right_temp_node,find_node=self.insert_node(value+self.max_distance*(count_iter+1))
        self.bulid_connection_left_to_right(node,right_temp_node)
        count_iter+=1

        while count_iter<self.block_num and not find_node:
            temp_node,find_node=self.insert_node(value+self.max_distance*(count_iter+1))
            self.bulid_connection_left_to_right(right_temp_node,temp_node)

            right_temp_node=temp_node
            count_iter+=1
            if count_iter==self.block_num:
                temp_node.isEdge=True
        count_iter=0

        left_temp_node,find_node=self.insert_node(value-self.max_distance*(count_iter+1))
        count_iter+=1
        self.bulid_connection_left_to_right(left_temp_node,node)
        while count_iter<self.block_num and not find_node:
            temp_node,find_node=self.insert_node(value-self.max_distance*(count_iter+1))
            self.bulid_connection_left_to_right(temp_node,left_temp_node)

            left_temp_node=temp_node
            count_iter+=1
            if find_node:
                break
            if count_iter==self.block_num:
    ...
    def insert_node(self, value):
        if not self.root:
            quotient=int(value/self.max_distance)
            if value<0:
                new_min_value=(quotient-1)*self.max_distance
                new_max_value=quotient*self.max_distance
            else:
                new_max_value=(quotient+1)*self.max_distance
                new_min_value=quotient*self.max_distance

            self.root = Node(mark_value=value,max_value=new_max_value,min_value=new_min_value, color=BLACK, parent=None, left=self.NIL_LEAF, right=self.NIL_LEAF)
            self.count += 1
        ...
        if node_dir == 'L':
            new_node = Node(mark_value=value,max_value=new_max_value, min_value=new_min_value, color=RED, parent=parent,
    ...
    def _remove_leaf(self, leaf):
        ...
        if leaf.mark_value >= leaf.parent.mark_value:
    ...
    def _try_rebalance(self, node):
        ...
        node_dir = 'L' if parent.mark_value > value else 'R'
        parent_dir = 'L' if grandfather.mark_value > parent.mark_value else 'R'
    ...
    def __update_parent(self, node, parent_old_child, new_parent):
        ...
        if new_parent:
            ...
            if new_parent.mark_value > parent_old_child.mark_value:
    ...
    def _find_parent(self, value):
        ...
        def inner_find(parent):
            ...
            """
            if parent.max_value >= value >= parent.min_value:
                return parent, None
            elif parent.max_value < value:
                if parent.right.color == NIL:  # no more to go
                    return parent, 'R'
                return inner_find(parent.right)
            elif value < parent.min_value:
    ...
    def find_node(self, value):
        def inner_find(root):
            if root is None or root == self.NIL_LEAF:
                return None
            if value > root.max_value:
                return inner_find(root.right)
            elif value < root.min_value:
    ...
    def _get_sibling(self, node):
        ...
        """
        parent = node.parent
        if node.mark_value >= parent.mark_value:
            sibling = parent.left
            direction = 'L'
        else:
    ...
    def all_points_from_tree(self,return_color=False,return_label=False):
        root_node=self.root
        all_points_set=set()
        node_q=Queue()
        node_q.put(root_node)
        while not node_q.empty():
            temp_node=node_q.get()
            all_points_set=all_points_set|temp_node.set_list[0]
            if temp_node.left.mark_value is not None:
                node_q.put(temp_node.left)
            if temp_node.right.mark_value is not None:
                node_q.put(temp_node.right)
        if return_color:
            node_point=np.zeros((len(all_points_set),6))
        if return_label:    
            node_point=np.zeros((len(all_points_set),4))

        point_count=0
        for point in all_points_set:
            if return_label:
                has_same=False
                for branch_node in point.branch_array:
                    if branch_node is not None and branch_node.pred_result==point.pred_result:
                        has_same=True
                        break
                if not has_same:
                    continue
                node_point[point_count,3]=point.pred_result

            node_point[point_count,:3]=point.point_coor


            if return_color:
                node_point[point_count,3:]=point.point_color

            point_count+=1
        return node_point[:point_count,]
