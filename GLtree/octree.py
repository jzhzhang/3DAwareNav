...
class GL_tree:
    ...
    def crf_points(self, per_image_node_set):
        ...
        if len(per_image_node_set) > 0 :
            ...
            d = dcrf.DenseCRF(len(per_image_node_set), self.num_sem_categories)
            
            U = np.zeros((self.num_sem_categories, len(per_image_node_set)), dtype=np.float32) # 7*N
    ...
    def sample_points(self):
        ...
        observation_points = np.zeros((4096, self.num_sem_categories + 3 + 1 +1)) #x,y,z, prob(7), kl_divergency, entropy
